// 워크플로우 통합 서비스
// 사전분석-제안-구축-운영 단계별 데이터 통합 및 전달

import { supabase } from '../../lib/supabase'
import { ProjectPhase } from '../lifecycle/ProjectLifecycleService'
import { preAnalysisService } from '../preAnalysis/PreAnalysisService'
import { ReportGenerator } from '../preAnalysis/ReportGenerator'

export interface BaselineData {
  projectId: string
  source: 'pre_analysis' | 'proposal' | 'analysis' | 'construction' | 'operation'
  version: string
  data: {
    // 사전 분석 단계 데이터
    preAnalysis?: {
...
export class WorkflowIntegrationService {
  ...
  /**
   * 사전 분석 단계 데이터를 베이스라인으로 설정
   */
  async setPreAnalysisBaseline(
    projectId: string,
    userId: string
    ...
    try {
      ...
      // 종합 보고서 생성 (메서드명 확인 필요)
      const comprehensiveReport = await reportGenerator.generateReport({
        projectId,
        sessionId: '',
        projectContext: { name: '', description: '', industry: '', techStack: [] },
        analysisResult: {
          summary: '',
          insights: [],
          recommendations: [],
        ...
        questions: [],
        answers: [],
        model: { provider: 'openai', id: 'gpt-4', name: 'GPT-4' } as any,
        depth: 'comprehensive',
        temperature: 0.6
      })
      ...
      const baseline: BaselineData = {
        ...
        data: {
          preAnalysis: {
            ...
            questionAnswers,
            aiInsights: (comprehensiveReport as any)?.insights || [],
            comprehensiveReport,
            keyFindings: (comprehensiveReport as any)?.findings || [],
            recommendations: (comprehensiveReport as any)?.recommendations || []
          }
        },
        metadata: {
          ...
          tags: ['pre_analysis', 'baseline'],
          confidenceScore: this.calculateConfidenceScore(comprehensiveReport)
        }
      }
      ...
      console.error('Failed to set pre-analysis baseline:', error)
      throw error
    }
  }
   ...
   * 사전 분석 완료 확인
   */
  async checkPreAnalysisCompletion(projectId: string): Promise<boolean> {
    try {
      const sessionResponse = await preAnalysisService.getActiveSession(projectId)
      if (!sessionResponse.success || !sessionResponse.data) return false

      const session = sessionResponse.data

      // 모든 단계가 완료되었는지 확인 (progress 구조 확인 필요)
      const requiredSteps = ['setup', 'analysis', 'questions', 'report']
      // session 구조에 따라 progress 확인 로직 수정 필요
      const progressData = session.metadata?.['progress'] || {}
      const completedSteps = Object.entries(progressData)
        .filter(([step, progress]) => requiredSteps.includes(step) && progress === 100)
        .map(([step]) => step)
  ...
  async transitionToProposal(
    ...
    try {
      // 사전 분석 완료 확인
      const isCompleted = await this.checkPreAnalysisCompletion(projectId)
      if (!isCompleted) {
        throw new Error('Pre-analysis is not completed yet')
      }

      // 사전 분석 베이스라인 설정
      const preAnalysisBaseline = await this.setPreAnalysisBaseline(projectId, userId)

      // 제안서 질문 생성
      const questionsGenerated = await this.generateProposalQuestions(
        projectId,
        preAnalysisBaseline.data.preAnalysis!
      )

      if (!questionsGenerated) {
        throw new Error('Failed to generate proposal questions')
      }
      ...
      const { error: updateError } = await supabase
        .from('projects')
        .update({
          current_workflow_step: 'proposal',
      ...
      await this.logWorkflowTransition({
        ...
        toStep: 'proposal',
        completionData: {
          preAnalysisReport: preAnalysisBaseline.data.preAnalysis?.comprehensiveReport
        },
   ...
   * 제안서 질문 생성 (사전 분석 기반)
   */
  async generateProposalQuestions(
    projectId: string,
    preAnalysisData: any
  ): Promise<boolean> {
    try {
      // 사전 분석 결과를 바탕으로 제안서 작성 질문 생성
      const proposalPrompt = this.buildProposalQuestionPrompt(preAnalysisData)
      ...
      const response = await fetch('/api/ai/completion', {
        ...
        body: JSON.stringify({
          messages: [
            {
              role: 'system',
              content: `당신은 사전 분석 결과를 바탕으로 제안서 작성을 위한 체계적인 질문을 생성하는 AI 어시스턴트입니다.`
            },
            {
              role: 'user',
      ...
      const generatedQuestions = JSON.parse(aiResult.content)

      // 제안서 질문을 데이터베이스에 저장
      await this.saveProposalQuestions(projectId, generatedQuestions)
  ...
  /**
   * 제안서 질문 생성 프롬프트 구축 (사전 분석 기반)
   */
  private buildProposalQuestionPrompt(preAnalysisData: any): string {
    return `
사전 분석 결과를 바탕으로 제안서 작성을 위한 체계적인 질문을 생성해주세요.

## 사전 분석 종합 보고서 요약
${preAnalysisData.comprehensiveReport?.summary || '사전 분석 요약 정보 없음'}

## 핵심 발견사항
${(preAnalysisData.keyFindings || []).map((finding: string) => `- ${finding}`).join('\n')}

## AI 인사이트
${(preAnalysisData.aiInsights || []).map((insight: string) => `- ${insight}`).join('\n')}

## 권장사항
${(preAnalysisData.recommendations || []).map((rec: string) => `- ${rec}`).join('\n')}

## 기존 질문-답변 요약
${(preAnalysisData.questionAnswers || []).map((qa: any) =>
  `Q: ${qa.question?.question_text || 'N/A'}\nA: ${qa.answer?.answer_text || 'N/A'}`
).join('\n\n')}

위 사전 분석 정보를 바탕으로 다음 카테고리별로 제안서 작성에 필요한 질문들을 JSON 형태로 생성해주세요:

1. 프로젝트 개요 및 목표 (사전 분석 결과 기반)
2. 기술적 접근 방법 (분석된 기술 요구사항 기반)
3. 일정 및 마일스톤 (복잡도 분석 기반)
4. 예산 및 리소스 (규모 분석 기반)
5. 위험 관리 및 대응 방안 (식별된 리스크 기반)
6. 성과 측정 및 평가 (권장사항 기반)
  ...
  "categories": [
    {
      ...
      "questions": [
        {
          "question_text": "질문 내용",
          "question_type": "text|textarea|select|multiselect|number",
          "options": ["옵션1", "옵션2"], // select/multiselect인 경우만
          "is_required": true|false,
          "help_text": "도움말 텍스트 (사전 분석 결과 참조)"
        }
      ]
    }
  ]
...
`;
   ...
   * 제안서 질문을 데이터베이스에 저장
   */
  private async saveProposalQuestions(projectId: string, questionsData: any): Promise<void> {
    try {
      const questions = []
      let displayOrder = 1
      ...
      for (const category of questionsData.categories) {
        for (const question of category.questions) {
          questions.push({
            ...
            question_id: `proposal_${Date.now()}_${displayOrder}`,
      ...
      console.error('Error saving proposal questions:', error)
      throw error
    }
  }
   ...
   * 워크플로우 전환 로그 기록
   */
  private async logWorkflowTransition(transitionData: {
    projectId: string
    fromStep: string
    toStep: string
    completionData: any
    ...
    try {
      ...
      // workflow_transitions 테이블이 없으므로 로그만 기록
      console.log('Workflow transition logged:', {
        project_id: transitionData.projectId,
        from_step: transitionData.fromStep,
        to_step: transitionData.toStep,
        completion_data: transitionData.completionData,
        transition_at: transitionData.transitionAt,
        triggered_by: transitionData.triggeredBy
      })
    } catch (error) {
      console.error('Error in logWorkflowTransition:', error)
    }
  }
